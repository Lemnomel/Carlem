...
class TASK
{
  ...
  void runtask(){
    for (int i=0;i<10;i++){
      if (!(taskcount[i]==0)){
        if(millis()>taskstart[i]){
          taskstart[i]=millis()+taskperiod[i];
          if (taskcount[i]>0){taskcount[i]=taskcount[i]-1;}
//          Serial2.println("Task: " + taskcommand[i] + " run");  
          Runcommand(taskcommand[i]);
        }
      }
    }
  }
...
//         if (!on)return true;
    ...
    void zikl(){
      ...
      Processingcommand();
      Processingsim900();              
////////      if(autoquery){Sendautoquery();} 
      if(flags&FLAG_autoquery){Sendautoquery();}       
      serialEvent();
      serialEvent1();
      serialEvent2();
      serialEvent3();
...
void Sendautoquery() {
//  if((millis()-timesendautoquery)>periodautoquery){
//   timesendautoquery=millis();  
...
void serialEvent1(){         //SIM900
  if (D){Serial.println("serialEvent1");}
...
void serialEvent2(){           //Bluetooth 
    if (D){Serial.println("serialEvent2");};
...
void serialEvent3(){       //ECU
  if (D){Serial.println("serialEvent3");};
    ...
    if(leghtpacket2!=0){
      ...
      if(byte(beginpacket2+leghtpacket2-1)==indpacket2){
        if(verifypacket2!=0){
          ...
          if (D){Serial.print("f1");};
          if (D){Serial.print("i"+String(indpacket2));};
          if (D){Serial.print("e"+String(endbuffer2));};                                                
          if (D){Serial.print("b"+String(beginpacket2));};
          if (D){Serial.print("l"+String(leghtpacket2));};                                                  
        }        
        if(verifypacket2==0){
...
void processpacket(){
   if (D){Serial.println("processpacket");};
...
void datatooled(int mode){
  ...
  char Stringtochar[datenow.length()+ 1];
  char Stringtochar1[timenow.length()+ 1];
  if (mode>0){LCD.lcdOn();} 
  Mytask.restarttask(2,2000);
  switch (mode) {
    case 0:
            LCD.lcdOff();
             break;      
    case 1:
            datenow="     " + String(now1.year()) + "/"
                              + String(now1.month()) + "/"
                              + String(now1.day()) + "       "; 
            timenow="     " + String(now1.hour()) + ":"
                              + String(now1.minute()) + ":"
                              + String(now1.second()) + "       ";
                                  
                       
             datenow.toCharArray(Stringtochar, datenow.length()+1);
             LCD.sendString(Stringtochar,0,0);
             timenow.toCharArray(Stringtochar1, timenow.length()+1);
             LCD.sendString(Stringtochar1,0,1);
    ...
    if (query.startsWith("And:")){
      ...
      if (query=="And:btpush"){
        ...
        if(Btstate){
          ...
          datatooledmode[getmode()]++;
          if (datatooledmode[getmode()]>10){
            datatooledmode[getmode()]=0;
          }          
          LCD.sendString("       MODE         ",0,0);
//          LCD.sendString("                    ",0,1);
          LCDsenddouble(datatooledmode[getmode()],0,0,13,0);
            switch (datatooledmode[getmode()]) {
              case 0:
...
void Fuelevent(){
  ...
  //     MPwrite(namefile()); 
       ...
       MPsend();
       MPwrite(namefile());
       if (D){Serial.println("-Fuelevent");} 
}

void Startevent(){
  if (D){Serial.println("Startevent");}
...
void LCDsenddouble(double number,unsigned int lennumber,unsigned int prezision,uint8_t col, uint8_t row){
  Serial2.println("Debug:" + String(number));
  char buffer1[10];
...
double Prm(){return buffer3[byte(beginpacket3+15)]*25;}
...
double TAir(){return(buffer3[byte(beginpacket3+10)]*0.75-40);}
double TWat(){return(buffer3[byte(beginpacket3+9)]*0.75-40);}    
double Speed(){return(buffer3[byte(beginpacket3+17)]);} //км/ч
double Lh(){//л/ч
  if (Prm()>0){
    Serial.println ("Lh=" + String (flow));
    return(flow);
  }
  else{
    return 0;
...
double LhAvr(){//л/ч
  if(Worktimeinginelong>0){
    Serial.println ("Volumelong=" + String (Volumelong));
    Serial.println ("Worktimeinginelong=" + String (Worktimeinginelong));
...
int getmode(){
  if (analogRead(ACCPin)<70){return 0;}
